...
router.post("/create-problems", async (req: Request, res: Response) => {
...
router.put("/update-problem/:id", async (req: Request, res: Response) => {
  ...
  if (!statement && !editorial) {
    res.status(400).json({ success: false, message: "No hay cambios para actualizar." });
  }

  try {
    await callProcedure("sp_update_problem", [id, statement, editorial]);

    res.json({ success: true, message: "Problema actualizado correctamente" });
  } catch (e: any) {
    console.error(e);
    res.status(500).json({ success: false, message: e.message });
  }
...
router.delete("/delete-problem/:id", async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.json({ success: true, message: "Problema eliminado correctamente" });
  } catch (e: any) {
    console.error(e);
    res.status(500).json({ success: false, message: e.message });
  }
